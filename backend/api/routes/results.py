    ...
    try:
        ...
        for v in variants:
            ...
            if uplift > best_uplift:
                best_uplift = uplift
                winner = v
                winner_confidence = "high" if p_val < 0.05 else "medium" if p_val < 0.1 else "low"
        ...
        for users_processed in sorted(grouped.keys()):
            ...
            for v in variants:
                ...
                std_proxy = abs(float(control["mean_metric"])) if abs(float(control["mean_metric"])) > 1e-6 else 1.0
...
async def _generate_segmentation_analysis(test_id: str, segment_by: str) -> Dict[str, Any]:
    with SessionLocal() as db:
        ...
        records = (dataset.extra_metadata or {}).get("records") or dataset.preview_json or []
        if not records:
